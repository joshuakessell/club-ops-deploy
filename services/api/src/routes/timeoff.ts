...
export async function timeoffRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post<{
    ...
    try {
      ...
      if (err?.code === '23505') {
        return reply.status(409).send({ error: 'A time off request already exists for that day.' });
      }
  ...
  fastify.patch<{
    ...
    try {
      const updated = await transaction(async (client) => {
        const current = await client.query<Pick<TimeOffRow, 'status' | 'employee_id' | 'day' | 'reason'>>(
